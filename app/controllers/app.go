...
func (c *App) requireUser() {
	if !c.isLogined() {
		...
		fmt.Println("current_user { id: ", c.currentUser.Id, ", login: ", c.currentUser.Login, " }")
	}
}
...
func (c App) validateCaptcha(code string) bool {
	captcha_id := c.Session["captcha_id"].(string)
	return captcha.VerifyString(captcha_id, code)
}
