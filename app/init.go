...
func init() {
	...
	if revel.RunMode == "prod" {
		train.Config.AssetsPath = "src/github.com/lenghan4real/mediom/app/assets"
	}
...
var AssetsFilter = func(c *revel.Controller, fc []revel.Filter) {
	if strings.HasPrefix(c.Request.URL.Path, "/assets") {
		train.ServeRequest(c.Response.Out.Server.GetRaw().(http.ResponseWriter), c.Request.In.GetRaw().(*http.Request))
	} else {
		fc[0](c, fc[1:])
	}
}
